...
def get_query(self, args, operation, field, start_date=None, end_date=None,
    ...
    if operation and field:
        ...
        if group_by:
            if group_by.ttype == 'many2one':
                ...
                data += ',"%s".%s' % (self._table, group_by.name)
    ...
    # Add filtering for x_deal_date
    if start_date and start_date != 'null':
        start_date_query = f' AND ({from_clause}."x_deal_date" >= \'{start_date}\')'
    else:
        start_date_query = ''
    
    if end_date and end_date != 'null':
        end_date_query = f' AND ({from_clause}."x_deal_date" <= \'{end_date}\')'
    else:
        end_date_query = ''
